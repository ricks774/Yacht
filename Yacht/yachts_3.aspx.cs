...
namespace Yacht
{
    public partial class yachts_3 : System.Web.UI.Page
    {
        ...
        private void LoadSpecification()
        {
            ...
            string sql = "select ys.Id, ym.Guid, ys.DetailSpecification from Yachts ys left join YachtsManager ym on ys.YachtsId = ym.Id where ym.Guid = @YachtGuid";
            SqlParameter[] parameters =
            {
            ...
            if (reader.Read())
            {
                Lr_LoadSpecification.Text = HttpUtility.HtmlDecode(reader["DetailSpecification"].ToString());
            }
        }
